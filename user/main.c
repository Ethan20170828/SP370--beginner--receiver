...
int main(void)
{
	...
	Display_Http();
	Display_Version();
	// 上面两个和下面两个Display_xxx函数，上电后值是不变的，
	// 所以先将这个四个初始化了，初始化的顺序就是屏幕中显示的先后顺序；
	//Display_User_ID_NULL();
	Display_Unique_ID_NULL();
	Display_RF_Receive_Mark();
	// 下面的这个四个Display_xxx，先将所有的值赋值为0，
	// 等待数据的传递，每传递一个数据，在屏幕上相应的位置刷新一下屏幕；
	Display_Pressure(0);
	...
	{
		// 如果有接收到数据(也就是TDA5235_FIFO_NUM不等于0),进行处理；
		// 接收的数据是从发射端发射过来的数据；
		if(TDA5235_FIFO_NUM){	
// 在中断处理程序中会对TDA5235_FIFO_NUM进行赋值(TDA5235通过一个GPIO来给我们一个中断，
// 在中断处理程序中读取TDA5235的FIFO，读完之后我们在根据读取到的FIFO的内容来给
// TDA5235_FIFO_NUM赋值)来影响我们这里的while(1)；

			// 将TDA5235_FIFO强制类型，然后赋值给pt_SP37Data，
			// 将TDA5235缓冲区中的数据传递给pt_SP37Data；
